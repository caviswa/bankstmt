...
class UniversalBankParser:
    ...
    def _find_headers(self, words: List[Dict]) -> Dict[str, Dict[str, float]]:
        ...
        for h_key, synonyms in self.header_synonyms.items():
            for word in words:
                ...
                matches = False
                for syn in synonyms:
                    syn_clean = re.sub(r'[^a-zA-Z]', '', syn).lower()
                    if syn_clean and syn_clean in text_clean:
                        matches = True
                        break
    ...
    def _process_page(self, words: List[Dict], running_balance: float = 0.0, last_headers: Dict = None) -> tuple:
        """
        Process a single page of words and applies multiline merging and math healing.
        Returns (processed_transactions, active_headers)
        """
        headers = self._find_headers(words)
        
        if not headers:
            headers = last_headers
            header_bottom = 0.0 # Table continues from top of page
        else:
            header_bottom = max([h['bottom'] for h in headers.values()])

        if not headers:
            return [], None # Completely unknown page formatting
        ...
        return processed_transactions, headers
    ...
    def parse_pdf(self, file_path: str) -> pd.DataFrame:
        ...
        with pdfplumber.open(file_path) as pdf:
            for page in pdf.pages:
                ...
                page_data, headers_used = self._process_page(words, running_balance=running_balance, last_headers=active_headers)
                
                if headers_used:
                    active_headers = headers_used
